...
parser.add_argument("--accumulate", type=int, default=0, choices=[0,1], help='If 1, search filebase directory for npy files, generate sparse matrix, and plot eigenvalues')
parser.add_argument("--mechanism", type=str, required=False, default='h2o2.cti', dest='mechanism', help='Mechanism cti file')
parser.add_argument("--rind", type=int, required=False, default=0, dest='rind', help='Reaction index')
parser.add_argument("--Nmax", type=int, required=False, default=1, dest='Nmax', help='Maximum number of molecules')
parser.add_argument("--Nvals", type=int, required=False, default=100, dest='Nvals', help='Number of eigenvalues')
...
for  i in range(Nmax**ns):
    stop=timeit.default_timer()
    print("%e\t%.2f\t%.2f"%(i*1.0/Nmax**ns, stop-start,(stop-start)*(Nmax**ns-i)/(i+1)),end="\t\r")
    ...
    if np.all((multiindex-pstoi)[np.where(pstoi>0)]>=0) and np.all(multiindex2<Nmax) and k>0:
        ...
        rows.append(i)
        columns.append(i)
print("%e\t%.2f\t%.2f"%(i*1.0/Nmax**ns, stop-start,(stop-start)*(Nmax**ns-i)/(i+1)))
#save to files
